...
const ProjectContext = () => {
  ...
  const { project, updateContext } = useProject(searchParams['id']);
  ...
  const submit = async () => {
    ...
    toast.promise(updateContext(JSON.stringify(finalResponse)), {
      loading: 'Submitting...',
      success: 'Submitted successfully',
      error: 'Something went wrong'
    }).then(() => {
      router.push(`/project/${searchParams['id']}`)
    })
    ...
    <div className="container mx-auto py-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {questionsNanswers.map((q, index) => (
          <QuestionCard index={index} setAnswer={setAnswer} key={q.question} question={q.question} answer={q.answer} description={q.description} />
        ))}
      </div>
      <button onClick={submit} className=" btn btn-primary text-white py-2 px-6 rounded-lg">Submit</button>
    </div>
  )
}
